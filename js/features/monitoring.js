export function createMonitoringFeature({
  ...
  const feature = {
    ...
    init() {
      ...
      byId('monitorSourceList')?.addEventListener('click', event => {
    ...
    render() {
      ...
      if (sourceList) {
        sourceList.innerHTML = store.state.sources.map(source => `
    ...
    openDataJudConfigModal() {
      ...
      ], { apiKey: currentKey, autoSync: 'active', tribunals: 'TJRS, TRF4, STJ, TST, TJSC, TJPR, TJSP' });
    },
