...
export function createDashboardFeature({
  ...
  const feature = {
          ...
          <div class="dashboard-task-item" data-dashboard-task-id="${escapeHtml(task.id)}">
            ...
            <div class="dashboard-task-body">
              ...
              <div class="dashboard-task-process">
                ...
                ${processNumber ? `<button type="button" class="dashboard-task-process-badge" data-dashboard-process-open="${escapeHtml(processNumber)}" title="Acessar processo">${iconSvg('processes')} <b>${escapeHtml(processNumber)}</b></button>` : '<span class="dashboard-task-context-empty">Processo não vinculado</span>'}
    ...
    actionableValues() {
      ...
      const publications = publicationsInRecentScope(state.intimations);
