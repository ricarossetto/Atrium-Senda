...
export class SecurityManager {
  ...
  async beginSetup({ username, displayName, email, password, bootstrapToken }, remoteAddress) {
  ...
  async finishSetup({ setupToken, code, skipMfa }) {
    ...
    await this.save();
    const session = this.createSession(this.state.username, this.state.displayName, 'master_admin', '');
  ...
  async login({ username, password, code, trustBrowser }, remoteAddress, userAgent = '') {
    ...
    const session = this.createSession(user.username, user.displayName, user.role, user.email, user.phone, user.avatar);
  ...
  createSession(username, displayName, role = 'master_admin', email = '', phone = '', avatar = '') {
    const token = randomBytes(32).toString('base64url');
    const csrfToken = randomBytes(24).toString('base64url');
    const now = Date.now();
    this.sessions.set(sha256(token), { username, displayName, role, email, phone: phone || '', avatar: avatar || '', csrfToken, createdAt: now, lastSeenAt: now, expiresAt: now + SESSION_MAX_AGE_MS });
    return { token, csrfToken, user: publicProfile({ username, displayName, role, email, phone, avatar }) };
  }
