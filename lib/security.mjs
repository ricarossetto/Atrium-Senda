...
export class SecurityManager {
  ...
  async verifyRegisteredUser({ setupToken, code, skipMfa }) {
    ...
    this.state.updatedAt = new Date().toISOString();
    await this.save();
    return { ok: true, status: user.status, mfaEnabled, recoveryCodes, message: 'Cadastro concluído com sucesso. A conta aguarda aprovação do administrador.' };
  }
  ...
  async beginSetup({ username, displayName, email, oab, oabUf, enableMonitoring, password, bootstrapToken }, remoteAddress) {
  ...
  async finishSetup({ setupToken, code, skipMfa }) {
    ...
    await this.save();
    const session = this.createSession(this.state.username, this.state.displayName, 'master_admin', pending.email || '', '', '', pending.oab || '', pending.oabUf || '', Boolean(pending.judicialMonitoringEnabled));
  ...
  async login({ username, password, code, trustBrowser }, remoteAddress, userAgent = '') {
    ...
    const session = this.createSession(user.username, user.displayName, user.role, user.email, user.phone, user.avatar, user.oab, user.oabUf, user.judicialMonitoringEnabled);
  ...
  createSession(username, displayName, role = 'master_admin', email = '', phone = '', avatar = '', oab = '', oabUf = '', judicialMonitoringEnabled) {
    const token = randomBytes(32).toString('base64url');
    const csrfToken = randomBytes(24).toString('base64url');
    const now = Date.now();
    this.sessions.set(sha256(token), { username, displayName, role, email, phone: phone || '', avatar: avatar || '', oab: oab || '', oabUf: oabUf || '', judicialMonitoringEnabled, csrfToken, createdAt: now, lastSeenAt: now, expiresAt: now + SESSION_MAX_AGE_MS });
    return { token, csrfToken, user: publicProfile({ username, displayName, role, email, phone, avatar, oab, oabUf, judicialMonitoringEnabled }) };
  }
