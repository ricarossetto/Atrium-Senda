...
export class SecurityManager {
  ...
  async beginSetup({ username, displayName, password, bootstrapToken }, remoteAddress) {
  ...
  async consumeRecoveryCodeForUser(user, code) {
    const salt = user.recoverySalt || this.state.recoverySalt;
    const hashes = user.recoveryHashes || this.state.recoveryHashes || [];
  ...
  async disableUserMfa(username, { password }) {
    let user = (this.state.users || []).find(u => u.username === username);
    if (!user && this.state.configured && constantEqual(username, this.state.username)) {
      user = this.state;
    }
  ...
  createSession(username, displayName, role = 'master_admin', email = '') {
