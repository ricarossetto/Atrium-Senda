...
export class RegistryService {
  ...
  status() {
    return {
      providers: [
        ...
        { id: 'cpf-external', name: 'Consulta externa de CPF', capabilities: ['CPF'], enabled: false, configured: false, priority: null, cacheTtlMs: null, state: 'not_configured', lastSuccessAt: null, lastLatencyMs: null }
      ],
      policy: {
        cpf: 'validation_only',
  ...
  validateDocument(value) {
    ...
    return {
      ...document,
      externalLookup: document.type === 'cpf' ? 'not_configured' : document.type === 'cnpj' ? 'available' : 'unsupported',
      message: document.type === 'cpf'
        ? 'Consulta externa de CPF não configurada. A validação local não confirma identidade nem situação cadastral.'
