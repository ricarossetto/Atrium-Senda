...
function makeEntry({ entityType, target = entityType, id, title, context = '', fields = [] }) {
  const safeId = visibleText(id, 300);
  const safeTitle = visibleText(title, 500);
  if (!safeId || !safeTitle || !Object.hasOwn(ENTITY_ORDER, entityType)) return null;
...
async function documentEntry(document, { contacts, processes, loadOcrText, ocrCache }) {
  ...
  return makeEntry({
    ...
    context: joinContext(owner, document.documentType, document.documentDate),
...
export async function buildSearchEntries(state = {}, {
  ...
  for (const item of state.intimations || []) entries.push(makeEntry({
    entityType: 'publication',
    target: 'intimation',
    id: item.id,
    title: item.title || 'Publicação',
    context: joinContext(item.process, item.client, item.court),
    fields: compactFields([
  ...
  for (const item of state.tasks || []) entries.push(makeEntry({
    entityType: 'task',
    id: item.id,
    title: item.title || 'Tarefa',
    context: joinContext(item.client, item.process, item.responsible),
    fields: compactFields([
      field('title', 'Título', item.title, 10),
