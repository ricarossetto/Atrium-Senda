...
const LIVE_COLLECTION_TIMEOUT_MS = 120_000;
...
export class TjrsSidecarClient {
  ...
  async getProcess(cnj, { accessKey = '' } = {}) {
    const digits = assertTjrsCnj(cnj);
    const payload = await this.#get(processEndpoint(`/v1/processes/${digits}`, accessKey), { notFoundCode: 'NOT_FOUND' });
  ...
  async getDiff(cnj, { accessKey = '' } = {}) {
    const digits = assertTjrsCnj(cnj);
    const payload = await this.#get(processEndpoint(`/v1/processes/${digits}/diff`, accessKey), { notFoundCode: 'NOT_FOUND' });
  ...
  async collectProcess(cnj, { accessKey = '', forceLive = true } = {}) {
    const digits = assertTjrsCnj(cnj);
    const key = String(accessKey || '').trim().slice(0, 500);
    const payload = await this.#post('/v1/processes/collect', {
      cnj: digits,
      forceLive: Boolean(forceLive),
      ...(key ? { accessKey: key } : {})
    });
    if (payload?.status !== 'success' || !payload?.data) {
      throw new TjrsSidecarError('O coletor TJRS não confirmou a primeira consulta do processo.', { code: 'INVALID_RESPONSE' });
    }
    return normalizeSnapshot(payload.data, digits);
  }
  ...
  async #post(pathname, body) {
    ...
    try {
      response = await this.fetchImpl(new URL(pathname, this.baseUrl), {
        ...
        redirect: 'error',
        signal: AbortSignal.timeout(LIVE_COLLECTION_TIMEOUT_MS)
      });
    } catch (error) {
      if (error?.name === 'TimeoutError' || error?.name === 'AbortError') {
        throw new TjrsSidecarError('A consulta ao TJRS demorou mais de dois minutos e foi interrompida.', { code: 'COLLECTION_TIMEOUT', statusCode: 504, cause: error });
      }
      throw new TjrsSidecarError('O coletor TJRS local está indisponível.', { code: 'UNAVAILABLE', statusCode: 503, cause: error });
    }

    const raw = await response.text().catch(() => '');
    if (Buffer.byteLength(raw, 'utf8') > MAX_RESPONSE_BYTES) {
      throw new TjrsSidecarError('O coletor TJRS devolveu uma resposta inválida.', { code: 'INVALID_RESPONSE' });
    }
    let payload = {};
    try { payload = raw ? JSON.parse(raw) : {}; }
    catch { throw new TjrsSidecarError('O coletor TJRS devolveu uma resposta inválida.', { code: 'INVALID_RESPONSE' }); }

    if (!response.ok) {
      if (payload?.isHumanActionRequired) {
        throw new TjrsSidecarError('O TJRS solicitou uma confirmação manual no navegador antes de liberar a consulta.', { code: 'HUMAN_ACTION_REQUIRED', statusCode: 409 });
      }
      if (response.status === 404) {
        throw new TjrsSidecarError('O processo não foi localizado no TJRS com os dados informados.', { code: 'PROCESS_NOT_FOUND', statusCode: 404 });
      }
      if ([400, 401, 403, 422].includes(response.status)) {
        throw new TjrsSidecarError('O TJRS não aceitou a chave informada. Confira os caracteres e tente novamente.', { code: 'ACCESS_KEY_REJECTED', statusCode: 422 });
      }
      throw new TjrsSidecarError('O coletor não conseguiu fazer a primeira consulta deste processo no TJRS.', { code: 'COLLECTION_FAILED', statusCode: response.status >= 500 ? 503 : 502 });
    }
    return payload;
  }
}

function processEndpoint(pathname, accessKey) {
  const key = String(accessKey || '').trim().slice(0, 500);
  if (!key) return pathname;
  const params = new URLSearchParams({ chaveAcesso: key });
