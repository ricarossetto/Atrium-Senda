...
export class JudicialCredentialManager {
  ...
  async getPortalCredentials(portalId) {
    const secrets = await this.readRawSecrets();
    return secrets.portalCredentials?.[portalId] || null;
  }

  async getPortalTotpSecret(portalId) {
    const secrets = await this.readRawSecrets();
    const entry = secrets.totpSecrets?.[portalId];
  ...
  async savePortalTotp(portalId, { secret, label = '' }) {
  ...
  async savePortalCredentials(portalId, { username, password, oab, uf }) {
    const secrets = await this.readRawSecrets();
    secrets.portalCredentials ||= {};
    secrets.portalCredentials[portalId] = {
