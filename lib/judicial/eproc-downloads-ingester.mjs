...
`;
  ...
  for (const d of extracted.deadlines || []) {
    ...
    const existingProc = (target.processes || []).find(p => (p.number || '').replace(/\D/g, '') === cnjNorm);
  ...
  for (const p of extracted.pending || []) {
    ...
    if (!existingInt) {
      const existingProc = (target.processes || []).find(proc => (proc.number || '').replace(/\D/g, '') === cnjNorm);
      const intClient = p.autor || existingProc?.client || '';
      target.intimations.unshift({
        id: intId,
        externalId: intId,
        processNumber: cnj,
