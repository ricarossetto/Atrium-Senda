...
function runCommand(cmd, args, stdinData = null) {
  return new Promise((resolve, reject) => {
...
async function validatePfxOpenSsl({ pfxPath, passphrase, challenge }) {
  try {
    const certPem = await runCommand('openssl', ['pkcs12', '-in', pfxPath, '-passin', `pass:${passphrase}`, '-nokeys']);
    const keyPem = await runCommand('openssl', ['pkcs12', '-in', pfxPath, '-passin', `pass:${passphrase}`, '-nocerts', '-nodes']);
    ...
    const tempCertPath = path.join(tmpdir(), `atrium-cert-${randomBytes(8).toString('hex')}.pem`);
    const tempKeyPath = path.join(tmpdir(), `atrium-key-${randomBytes(8).toString('hex')}.pem`);

    await writeFile(tempCertPath, certPem);
    await writeFile(tempKeyPath, keyPem);

    await runCommand('openssl', ['pkcs12', '-export', '-out', tempModernPath, '-inkey', tempKeyPath, '-in', tempCertPath, '-passout', `pass:${modernPwd}`]);
