  ...
  try {
    ...
    try {
      ...
      for (const item of allIntimations) {
        ...
        const exists = state.intimations.some(i => (i.processNumber || i.process || '').replace(/\D/g, '') === cnjNorm && (i.event === item.event || i.description === item.subject));
        if (!exists) {
          state.intimations.push({
            id: `intimation:eproc:${Date.now()}-${randomBytes(4).toString('hex')}`,
            ...
            date: item.sentAt || item.startsAt || nowIso.slice(0, 10),
      ...
      if (newIntimationsCount > 0) {
        console.log(`[eproc sweep] ✓ ${newIntimationsCount} nova(s) intimação(ões) importada(s) do painel do advogado no eproc.`);
      }
    } catch (intimationErr) {
    ...
    let nextRevision = envelopeRevision;
    if (updatedProcesses.length > 0 || newIntimationsCount > 0) {
